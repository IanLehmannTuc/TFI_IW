...
public class IngresoRepositoryPostgres implements IngresoRepository {
    ...
    private class IngresoRowMapper implements RowMapper<Ingreso> {
        ...
        private Paciente mapPaciente(ResultSet rs) throws SQLException {
            ...
            if (nombre == null) {
                return new Paciente(cuil, domicilio, afiliado);
            }
        ...
        private Usuario mapUsuario(ResultSet rs, String prefix) throws SQLException {
            ...
            return new Usuario(emailVO, passwordHash, autoridad, cuilVO, nombre, apellido, matricula);
        }
    }
    ...
    public Ingreso add(Ingreso ingreso) {
        ...
        // Generar ID único
        String ingresoId = UUID.randomUUID().toString();
        ingreso.setId(ingresoId);

        String sql = "INSERT INTO ingresos (id, paciente_cuil, enfermero_email, doctor_email, " +
        ...
        jdbcTemplate.update(sql,
            ...
            ingreso.getEnfermero() != null ? ingreso.getEnfermero().getEmail().getValue() : null,
            ingreso.getAtencion() != null && ingreso.getAtencion().getMedico() != null ? 
                ingreso.getAtencion().getMedico().getEmail().getValue() : null,
    ...
    public Ingreso update(Ingreso ingreso) {
        ...
        String sql = "UPDATE ingresos SET paciente_cuil = ?, enfermero_email = ?, doctor_email = ?, " +
                     "descripcion = ?, informe_doctor = ?, fecha_hora_ingreso = ?, temperatura = ?, " +
                     "presion_sistolica = ?, presion_diastolica = ?, frecuencia_cardiaca = ?, " +
                     "frecuencia_respiratoria = ?, nivel_emergencia = ?::text, estado = ?::text " +
                     "WHERE id = ?";
    ...
    public Optional<Ingreso> findById(String ingresoId) {
        ...
        String sql = buildSelectQuery() + " WHERE i.id = ?";
    ...
    private boolean existsById(String ingresoId) {
        String sql = "SELECT COUNT(*) FROM ingresos WHERE id = ?";
    ...
    private String buildSelectQuery() {
        return "SELECT " +
               ...
               // Paciente
               "p.cuil AS paciente_cuil, p.nombre AS paciente_nombre, " +
               ...
               "os.nombre_obra_social AS paciente_nombre_obra_social, " +
               // Enfermero (Usuario)
               "e.email AS enfermero_email, e.password_hash AS enfermero_password_hash, " +
               ...
               // Doctor (Usuario)
               "d.email AS doctor_email, d.password_hash AS doctor_password_hash, " +
               ...
               "FROM ingresos i " +
               "INNER JOIN pacientes p ON i.paciente_cuil = p.cuil " +
               "INNER JOIN usuarios e ON i.enfermero_email = e.email " +
               "LEFT JOIN usuarios d ON i.doctor_email = d.email " +
               "LEFT JOIN obras_sociales os ON p.obra_social_id = os.id_obra_social";
    }
}
