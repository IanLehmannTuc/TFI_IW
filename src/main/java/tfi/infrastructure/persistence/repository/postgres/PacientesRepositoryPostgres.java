...
public class PacientesRepositoryPostgres implements PacientesRepository {
    ...
    public List<Paciente> findAll() {
        String sql = "SELECT p.cuil, p.nombre, p.apellido, p.email, " +
                     "p.domicilio_calle, p.domicilio_numero, p.domicilio_localidad, " +
                     "p.obra_social_id, p.numero_afiliado, os.nombre_obra_social " +
                     "FROM pacientes p " +
    ...
    private Integer obtenerOCrearObraSocial(ObraSocial obraSocial) {
        ...
        String sqlSelect = "SELECT id_obra_social FROM obras_sociales WHERE nombre_obra_social = ?";
        List<Integer> results = jdbcTemplate.query(sqlSelect, 
            (rs, rowNum) -> rs.getInt("id_obra_social"),
            obraSocial.getNombreObraSocial()
        );
        ...
        String sqlInsert = "INSERT INTO obras_sociales (nombre_obra_social) VALUES (?) RETURNING id_obra_social";
        return jdbcTemplate.queryForObject(sqlInsert, Integer.class, obraSocial.getNombreObraSocial());
    }
}
